...
storm_lifeDamages <- storm %>% 
    ...
    mutate(allLifeDamages = FATALITIES + INJURIES, EVTYPE = toupper(EVTYPE)) %>%
    filter(allLifeDamages > 0) #%>%
    #group_by(EVTYPE) #%>%
...
eventNames <- unique(sort(storm_lifeDamages2$EVTYPE))
...
storm_lifeDamages3 <- storm_lifeDamages2 %>% 
    ...
    mutate(totalDamage = sum(allLifeDamages), medianDamage = median(allLifeDamages), count = n()) %>%
    arrange(desc(allLifeDamages))
#     group_by(cleanedEventNames) %>%
...
library(ggplot2)
g <- ggplot(data= storm_lifeDamages3, aes(x = reorder(cleanedEventNames, -totalDamage), y=log10(allLifeDamages)))
...
g <- g + geom_point(aes(y = log10(medianDamage), size = count, color =count)) +
    ...
    labs(x="Event Type; arranged by total damage to life over years (high to low)", 
         y="log10 (harm to human life)") + 
